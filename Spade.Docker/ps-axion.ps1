...
function Execute-Run {
    ...
    docker run -d --network host --name spade gspalato/spade
    Write-Host -NoNewline "`n"
}
...
function Execute-Stop {
    ...
    docker stop spade spade_database_1 spade_lavalink_1
    docker rm spade spade_database_1 spade_lavalink_1
