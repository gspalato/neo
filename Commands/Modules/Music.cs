...
namespace Muon.Commands.Modules
{
	...
	public sealed class Music : MuonModule
	{
			...
			if (_lavaNode.TryGetPlayer(Context.Guild, out var player))
			{
				if (player.VoiceChannel.Id != Context.User.VoiceChannel.Id)
				{
					...
					else if (users.Count == 1)
					{
						...
						player = await _lavaNode.JoinAsync(Context.User.VoiceChannel, Context.Channel as ITextChannel);
					}
				}
			}
			else
		...
		public async Task QueueAsync()
		{
			...
			Embed[] pages = new Embed[] { };

			var queue = player.Queue.Items.Chunk(7);

			int totalTrackNumber = 0;
			for (int chunkNumber = 0; chunkNumber < queue.Count(); chunkNumber++)
			{
				var chunk = queue.ElementAt(chunkNumber);
				string description = "";

				for (int trackNumber = 0; trackNumber < chunk.Count(); trackNumber++)
				{
					var track = chunk.ElementAt(trackNumber) as LavaTrack;
					description += $"{totalTrackNumber}. [**{track.Title.TruncateAndEscape()}**]({track.Url})";
					totalTrackNumber++;
				}

				Embed embed = new EmbedBuilder()
					.WithTitle($":musical_score:  Queue | Page {chunkNumber}/{queue.Count()}")
					.WithDefaultColor()
